...
Before moving on to the next algorithm, let us also plot the clusters against the data points to see how well the data is separated. To visualize the data in two dimensions, we will also make use of a dimensionality reduction technique, in this case PCA, to 

```{python}
#| echo: false
from sklearn.decomposition import PCA, KernelPCA

pca = KernelPCA(n_components = 2, kernel='rbf').set_output(transform = 'polars')
#pca = PCA(n_components = 2).set_output(transform = 'polars')
...
X_pca = X_pca.with_columns(labels = pl.Series(kmeans.labels_))
...
X_tsne = X_tsne.with_columns(labels = pl.Series(kmeans.labels_))
...
```{python}
(
    ggplot(X_tsne, aes(x='tsne0', y='tsne1', colour='labels')) + 
    geom_point()
)
```
...
```{python}
(
    ggplot(X_pca, aes(x='pca0', y='pca1', colour='labels')) + 
    geom_point()
)
```
...
(
    ggplot(X_pca, aes(x='kernelpca0', y='kernelpca1', colour='labels')) + 
    geom_point()
)
...
X_tsne = X_tsne.with_columns(labels = pl.Series(dbscan.labels_))
...
```{python}
X_pca = X_pca.with_columns(labels = pl.Series(dbscan.labels_))
